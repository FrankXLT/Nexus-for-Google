...
## 6.6 The Adaptive Mobile Law (Responsive Degradation)
The SPA MUST feel like a native application on mobile devices (`< 768px` width). The coding agent MUST utilize Tailwind's responsive prefixes (`md:`, `lg:`) to gracefully degrade complex visualizations. Horizontal scrolling of the entire viewport is strictly FORBIDDEN.

1. **Drawer-First Modals:** All popup interfaces (Context Modal, AI Inspector, System Settings) MUST utilize responsive components (e.g., `shadcn/ui` Dialog/Drawer). On desktop, they render as centered modals. On mobile, they automatically morph into swipeable **Bottom Sheets** rooted to the bottom of the screen.
2. **Grid-to-Card Transformation:** Standard `<table>` grids (Staging Grid, Taxonomy Matrix) are FORBIDDEN from relying on horizontal scrolling on mobile. At the `md:` breakpoint, TanStack tables MUST switch their render output from a table layout to a vertical list of stacked `<Card>` components. Where applicable (e.g., Staging queues), these mobile cards should support native Left/Right swipe gestures for rapid triage.
3. **VQB Collapsing & Orientation:** To preserve vertical real estate on mobile, the VQB (Heatmap and Sankey) MUST default to a collapsed state, hidden behind a toggleable "Insights" button beneath the Omnibox. When expanded on mobile, the ECharts Sankey MUST dynamically shift its `orient` property from `horizontal` to `vertical`, and the Heatmap MUST be wrapped in a horizontally scrollable container (`overflow-x-auto`).
4. **Touch-Safe React Flow:** Complex drag-and-drop interactions suffer from touch-event conflicts on mobile. The React Flow canvas MUST switch to `nodesDraggable={false}` (Read/Pan-Only) on mobile breakpoints. Tapping a node on mobile MUST open a Bottom Sheet Drawer containing standard `<select>` dropdowns to execute re-parenting actions. Visual drag-and-drop re-parenting is strictly defined as a Desktop-Class action.
5. **Sticky Action Bar:** The Zustand "Draft State" commit bar (Undo/Submit changes) MUST become a sticky, full-width bar at the very bottom of the mobile viewport (`bottom-0`, `w-full`, high `z-index`) to remain easily accessible to the user's thumbs.